...
type Action =
  ...
  | { type: 'MOVE'; from: Selection; to: Selection }
  | { type: 'UNDO' }
  | { type: 'SHOW_HINT' }
  | { type: 'CLEAR_HINT' }
...
function reducer(state: SolitaireState, action: Action): SolitaireState {
  switch (action.type) {
    ...
    case 'DRAW': {
      ...
      return {
        ...state,
        history: pushHistory(state.history, newState),
        selection: null,
        hint: null,
        isWon: false,
      };
...
export function useSolitaireGame() {
  ...
  const move = useCallback((from: Selection, to: Selection) => dispatch({ type: 'MOVE', from, to }), []);
